...
import model.*;
import model.threads.AFKCheckerThread;
import model.threads.GameThread;
import model.threads.MainThread;
import model.threads.MessagesRepeaterThread;
...
public class MasterThread extends Thread implements GameThread {
    ...
    public void run(){
        while (isRunning){
            ...
            switch (gameMessage.getTypeCase()){
                case STEER:{
                    fieldInfo.addMove(new Move(gameMessage.getSteer().getDirection(), gameMessage.getSenderId()));
                    SnakesProto.GameMessage answer = SnakesProto.GameMessage.newBuilder()
                            .setAck(
                                    SnakesProto.GameMessage.AckMsg.getDefaultInstance()
                            )
                ...
                case ROLE_CHANGE:{
                    ...
                    if(gameMessage.getRoleChange().getSenderRole() == SnakesProto.NodeRole.VIEWER) {
                        if (player.getRole() == SnakesProto.NodeRole.DEPUTY) {//need new deputy
    ...
    public void makeMove(SnakesProto.Direction direction){
        //send message STEER
        fieldInfo.addMove(new Move(direction, ID));
    }
